...
public class DivingLogStayEntryFragment extends Fragment {
    ...
    public final static String KEY_OBJ_ADDEDITTEXTS = "addEditTexts";
    public final static String KEY_OBJ_NONADDEDITTEXTS = "nonaddEditTexts";
    ...
    private void initializeBundle(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null && !savedInstanceState.isEmpty()) {
            ...
            if (currentId != MainActivity.me.currentDive.getDiveNr()) {
                addEditTexts.clear();
                nonaddEditTexts.clear();
            } else {
                if (savedInstanceState.getSerializable(KEY_OBJ_ADDEDITTEXTS) != null) {
                    addEditTexts = (SerializableSparseArray<WheelDrivenProfilePartEditText>) savedInstanceState.getSerializable(KEY_OBJ_ADDEDITTEXTS);
                }
                if (savedInstanceState.getSerializable(KEY_OBJ_NONADDEDITTEXTS) != null) {
                    nonaddEditTexts = (SerializableSparseArray<WheelDrivenProfilePartEditText>) savedInstanceState.getSerializable(KEY_OBJ_NONADDEDITTEXTS);
                }
            }
        }
    }
    ...
    private void addTableRowForPart(TableLayout table, ProfilePart part,
        ...
        if (list != null && list.get(part.orderNumber) != null) {
            WheelDrivenProfilePartEditText wdppEditText = list.get(part.orderNumber);
            wdppEditText.numberEditText=editText;
            wdppEditText.currentPart=dpart;
            wdppEditText.currentDive=MainActivity.me.currentDive;
        } else {
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putSerializable(MainActivity.KEY_SER_DIVE,
                MainActivity.me.currentDive);
        if (addEditTexts != null) {
            outState.putSerializable(KEY_OBJ_ADDEDITTEXTS, addEditTexts);
        }
        if (nonaddEditTexts != null) {
            outState.putSerializable(KEY_OBJ_NONADDEDITTEXTS, nonaddEditTexts);
        }
    }
