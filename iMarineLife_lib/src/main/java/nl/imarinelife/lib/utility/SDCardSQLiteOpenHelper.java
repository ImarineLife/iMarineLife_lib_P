...
public abstract class SDCardSQLiteOpenHelper extends SQLiteOpenHelper {
	...
	private static File			databasesSdDirectory	= null;
	 ...
	 */
	public SDCardSQLiteOpenHelper(Context context, String dir, String name, CursorFactory factory, int version) {
	...
	public synchronized SQLiteDatabase getReadableDatabase() {
		...
		try {
			...
			Log.d(TAG,
				"Couldn't open " + mName + " for writing (will try read-only):",
				e);
		}
	...
	public static boolean isSDCardiMarineLifeDirectoryActive() {
		Boolean isSDPresent = android.os.Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED);
		if (isSDPresent) {
			getDataBasesDirectory();
			if (databasesSdDirectory.exists()) {
				return true;
			} else {
				return false;
			}
	...
	public static String getDataBasesDirectory() {
		if (databasesSdDirectory != null && databasesSdDirectory.exists())
			return databasesSdDirectory.getAbsolutePath();
		else {
				databasesSdDirectory = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "iMarineLife");
				databasesSdDirectory = new File(databasesSdDirectory, "shared_databases");
				makeDirectoryStructureRecursive(databasesSdDirectory);
		}
		return databasesSdDirectory.getAbsolutePath();
	}
	
	private static void makeDirectoryStructureRecursive(File goal) {
		if (!goal.getParentFile().exists()) {
			makeDirectoryStructureRecursive(goal.getParentFile());
	}
		goal.mkdir();
